...
export class TrialsArticleAllergan extends Component {
  static propTypes = {
  ...
  render() {
    const { index, addr, trial, landingcoming, utm } = this.props;
    let utmPart = '';
    if (utm) {
      utmPart = `?utm=${utm}`;
    }
    const landingHref = `/${trial.study_id}-${trial.url_part.toLowerCase().replace(/ /ig, '-')}${utmPart}`;
    ...
    const phoneNumber = trial.click_to_call_button_number || trial.mlpphone || trial.recruitment_phone || null;
...
export default connect(null, mapDispatchToProps)(TrialsArticleAllergan);
