...
} from '../../containers/HelpSupportPage/actions';

import {
  SUBMIT_FORM,
  FETCH_COMPANY_TYPES,
} from '../../containers/HelpSupportPage/constants';
...
export function* submitFormWatcher() {
  while (true) {
    ...
    try {
      const requestURL = `${API_URL}/helpSupportral`;
      const params = {
        method: 'POST',
      ...
      yield put(toastrActions.success('HelpSupport', 'The request has been submitted successfully'));
      yield put(formSubmitted(response));

      // Clear the form values
      yield put(reset('helpSupport'));
    } catch (err) {
...
export function* helpSupportPageSaga() {
