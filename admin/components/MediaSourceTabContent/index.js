...
export default class MediaSourceTabContent extends Component { // eslint-disable-line react/prefer-stateless-function
  ...
  render() {
    const { studies, active, type, paginationOptions, loadItems } = this.props;
    let key = 0;
    if (active) {
      return (
        <InfiniteScroll
        ...
        >
          <table id="mediaSourceTabContent">
            ...
            <tbody>
              ...
              {studies.fetching &&
                <tr className="loading"><td colSpan={15} className="text-center"><LoadingSpinner showOnlyIcon /></td></tr>
              }
            </tbody>
          </table>
        </InfiniteScroll>
